...
function filtrar() {
    router.get('/maquinas/no-visibles', { pais: pais.value }, { preserveScroll: true });
}
...
function onCentroChange() {
    const found = centros.value.find((c) => c.IdCentro === activateForm.value.idCentro);
    if (found) {
        activateForm.value.country = countryMap[pais.value.toLowerCase()] || 3;
    }
}
...
async function eliminarBajaFisica(idMaquina: number) {
    try {
        const res = await fetch('/api/maquinas/baja-y-eliminar', {
            method: 'POST',
        ...
        const data = await res.json();
        if (data.success) {
            toast('Máquina eliminada permanentemente', 'success');
...
<template>
    ...
    <Layout>
        <div class="min-h-screen bg-slate-50 font-sans text-slate-700">
            <div class="mx-auto max-w-[1900px] px-2 py-4 sm:px-4 sm:py-6">
                <header class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <p class="mb-1 text-xs font-semibold tracking-[0.2em] text-red-500 uppercase">Máquinas</p>
                        <h1 class="text-2xl font-black tracking-tight text-slate-800 sm:text-3xl">
                            Listado <span class="text-red-400">No Visibles</span>
                        </h1>
                ...
                <div class="mb-4 grid grid-cols-3 gap-3">
                    ...
                    <div class="rounded-xl border border-red-200 bg-red-50 p-4">
                        <p class="text-[10px] tracking-wider text-red-600 uppercase">No Visibles</p>
                        <p class="mt-1 text-2xl font-black text-red-600">{{ maquinasCount }}</p>
                    </div>
                </div>
                    ...
                    <table class="w-full border-collapse text-sm">
                        <thead>
                            <tr class="border-b border-slate-200 bg-slate-50">
                                ...
                                <th class="w-16 px-3 py-3 text-center text-[10px] font-bold tracking-widest text-slate-500 uppercase">Eliminar</th>
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td class="px-3 py-2.5 text-center">
                                    <span v-if="Number(row.Relay) === 0" class="inline-flex items-center rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold text-emerald-700">Verde</span>
                                    <span v-else class="inline-flex items-center rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-semibold text-blue-700">Azul</span>
                                </td>
                ...
                <p v-if="!datos.length" class="py-16 text-center text-slate-400">
                    No hay máquinas no visibles para {{ paisNombre[pais] || pais }}.
                </p>
            </div>
        </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button
                                @click="confirmToggle(
                                    Number(modalData!.esVisible) ? '¿Ocultar esta máquina? Ya no aparecerá en los reportes.' : '¿Hacer visible esta máquina?',
        ...
        >
            <div class="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-lg font-bold text-slate-800">Activar Máquina</h3>
                    <button @click="closeActivateModal" class="text-slate-400 hover:text-slate-600">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        >
                            ...
                            <option v-for="c in centros" :key="c.IdCentro" :value="c.IdCentro">{{ c.NombreCentro }}</option>
                        </select>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-600">Country ID</label>
                        <input v-model="activateForm.country" type="number" readonly class="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-500" />
                        <p class="mt-1 text-[10px] text-slate-400">Se asigna automáticamente según el centro</p>
                    </div>
                </div>
