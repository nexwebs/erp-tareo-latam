...
function filtrar() {
    router.get('/produccion/chile', { fecha_inicio: fechaInicio.value, fecha_fin: fechaFin.value }, { preserveScroll: true });
}

function exportar() {
    window.location.href = `/produccion/exportar?fecha=${fechaFin.value}&pais=chile`;
}
    ...
    <div class="min-h-screen bg-[#080c18] font-sans text-white">
        <div class="mx-auto max-w-[1700px] px-4 py-6">
            ...
            <header class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <p class="mb-1 text-xs font-semibold uppercase tracking-[0.2em] text-sky-500">
                        Producción — Chile
                    </p>
            ...
            <div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <div class="rounded-xl border border-sky-900/50 bg-sky-900/20 p-4">
                    <p class="text-xs uppercase tracking-wider text-sky-600">Total</p>
                    <p class="mt-1 text-2xl font-black text-sky-400">{{ fmt(totalGeneral) }}</p>
                </div>
                ...
                <table class="w-full min-w-max border-collapse text-sm">
                    <thead>
                        <tr class="border-b border-slate-800">
                            ...
                            <th class="border-r border-slate-800 bg-sky-950/40 px-3 py-3 text-center text-[10px] font-bold uppercase tracking-widest text-sky-500">Total</th>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="border-r border-slate-800 bg-sky-950/20 px-3 py-2 text-right text-xs font-bold tabular-nums text-sky-400">{{ fmt(row.total) }}</td>
                    ...
                    <tfoot>
                        <tr class="border-t-2 border-slate-700 bg-slate-800/60">
                            ...
                            <td
                                v-for="h in horas"
                                :key="h"
                                class="border-r border-slate-700/50 px-2 py-3 text-center text-xs font-semibold tabular-nums text-sky-400"
                            >
                                {{ fmt(totalesHora[h]) }}
                            </td>
                            <td class="border-r border-slate-700 bg-sky-950/40 px-3 py-3 text-right text-sm font-black tabular-nums text-sky-300">{{ fmt(totalGeneral) }}</td>
