...
function filtrar() {
    router.get('/produccion/colombia', { fecha: fecha.value }, { preserveScroll: true });
}

function exportar() {
    const url = `/produccion/colombia/pdf?fecha=${fecha.value}`;
    const printWindow = window.open(url, '_blank');
    printWindow?.addEventListener('load', () => {
        printWindow.document.title = `Producción Colombia - ${fecha.value}`;
        printWindow.print();
        ...
        <div class="min-h-screen bg-slate-50 font-sans text-slate-700">
            <div class="w-full px-2 py-4 sm:px-4 sm:py-6">
                <header class="mb-6 flex flex-col gap-3 sm:mb-8 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <p class="mb-1 text-xs font-semibold tracking-[0.2em] text-rose-500 uppercase">
                            Producción — Colombia
                        </p>
                        <h1 class="text-2xl font-black tracking-tight text-slate-700 sm:text-3xl">
                            Reporte <span class="text-rose-500">por Centro</span>
                        </h1>
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <input
                            v-model="fecha"
                            type="date"
                            class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 focus:border-rose-500 focus:outline-none"
                        />
                        <button
                            @click="filtrar"
                ...
                <div class="mb-4 grid grid-cols-2 gap-3 sm:mb-6 sm:grid-cols-4">
                    ...
                    <div class="rounded-xl border border-rose-200 bg-rose-50 p-3 sm:p-4">
                        <p class="text-[10px] tracking-wider text-rose-600 uppercase sm:text-xs">Total</p>
                        <p class="mt-1 text-xl font-black text-rose-600 sm:text-2xl">{{ fmt(totalGeneral) }}</p>
                    </div>
                ...
                <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <input
                        ...
                        class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 placeholder-slate-400 focus:border-rose-500 focus:outline-none sm:w-80 sm:py-3"
                    />
                    ...
                    <table class="w-full border-collapse text-sm" style="table-layout: auto;">
                        ...
                        <thead>
                            <tr class="border-b border-slate-200 bg-white">
                                ...
                                >{{ h }}h</th>
                                <th class="bg-rose-50 px-3 py-3 text-center text-[10px] font-bold tracking-widest text-rose-600 uppercase whitespace-nowrap">
                                    Total
                                </th>
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                >{{ row.centro }}</td>
                                <td
                                    class="z-10 border-r border-slate-200 bg-white px-3 py-2 text-center font-mono text-xs text-rose-400 whitespace-nowrap"
                                    :style="{ position: 'sticky', left: stickyLeft.serie + 'px' }"
                                >{{ row.serie }}</td>
                                <td
                                    v-for="h in horas"
                        ...
                        <tfoot>
                            <tr class="border-t-2 border-slate-200 bg-slate-50">
                                ...
                                <td
                                    v-for="h in horas"
                                    :key="h"
                                    class="border-r border-slate-100 px-2 py-3 text-center text-xs font-semibold text-rose-500 tabular-nums whitespace-nowrap"
                                >{{ fmt(totalesHora[h]) }}</td>
                                <td class="bg-rose-50 px-3 py-3 text-right text-sm font-black text-rose-600 tabular-nums whitespace-nowrap">
                                    {{ fmt(totalGeneral) }}
                                </td>
