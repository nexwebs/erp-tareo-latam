...
function filtrar() {
    router.get('/produccion/colombia', { fecha_inicio: fechaInicio.value, fecha_fin: fechaFin.value }, { preserveScroll: true });
}

function exportar() {
    window.location.href = `/produccion/exportar?fecha=${fechaFin.value}&pais=colombia`;
}
...
function intensidad(valor: number, maxHora: number): string {
    ...
    if (pct >= 0.8) return 'text-rose-300 font-semibold';
    if (pct >= 0.5) return 'text-rose-400';
    if (pct >= 0.2) return 'text-rose-600';
    ...
    <div class="min-h-screen bg-[#080c18] font-sans text-white">
        <div class="mx-auto max-w-[1700px] px-4 py-6">
            ...
            <header class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <p class="mb-1 text-xs font-semibold uppercase tracking-[0.2em] text-rose-500">
                        Producción — Colombia
                    </p>
                    <h1 class="text-3xl font-black tracking-tight text-white">
                        Reporte <span class="text-rose-400">por Centro</span>
                    </h1>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <input
                        v-model="fechaInicio"
                        type="date"
                        class="rounded-lg border border-slate-700 bg-slate-800/80 px-3 py-2 text-sm text-white focus:border-rose-500 focus:outline-none"
                    />
                    <input
                        v-model="fechaFin"
                        type="date"
                    ...
                    <button
                        @click="filtrar"
                        class="rounded-lg bg-rose-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-rose-500 active:scale-95"
                    >
                        Filtrar
                    </button>
                    <button
            ...
            <div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <div class="rounded-xl border border-rose-900/50 bg-rose-900/20 p-4">
                    <p class="text-xs uppercase tracking-wider text-rose-600">Total</p>
                    <p class="mt-1 text-2xl font-black text-rose-400">{{ fmt(totalGeneral) }}</p>
                </div>
                ...
                <div class="rounded-xl border border-slate-800 bg-slate-800/40 p-4">
                    <p class="text-xs uppercase tracking-wider text-slate-500">Centros</p>
                    <p class="mt-1 text-2xl font-black text-white">{{ centros.length }}</p>
                </div>
            </div>
            ...
            <div class="mb-4">
                <input
                    ...
                    class="w-full rounded-xl border border-slate-700 bg-slate-800/60 px-4 py-3 text-sm text-white placeholder-slate-600 focus:border-rose-500 focus:outline-none sm:w-80"
                />
            </div>
                ...
                <table class="w-full min-w-max border-collapse text-sm">
                    <thead>
                        <tr class="border-b border-slate-800">
                            ...
                            <th class="border-r border-slate-800 bg-rose-950/40 px-3 py-3 text-center text-[10px] font-bold uppercase tracking-widest text-rose-500">Total</th>
                    ...
                    <tbody>
                        ...
                        >
                            <td class="sticky left-0 z-10 border-r border-slate-800/50 bg-[#080c18] px-3 py-2 text-center text-xs text-slate-600">{{ row.item }}</td>
                            <td class="sticky left-10 z-10 w-[120px] max-w-[120px] truncate border-r border-slate-800/50 bg-[#080c18] px-3 py-2 text-xs text-slate-300">{{ row.modelo }}</td>
                            <td class="sticky left-[130px] z-10 w-[180px] max-w-[180px] truncate border-r border-slate-800/50 bg-[#080c18] px-3 py-2 text-xs text-slate-300">{{ row.centro }}</td>
                            <td class="sticky left-[310px] z-10 border-r border-slate-800/50 bg-[#080c18] px-3 py-2 text-center font-mono text-xs text-cyan-400">{{ row.serie }}</td>
                            <td
                                v-for="h in horas"
                                :key="h"
                            ...
                            <td class="border-r border-slate-800 bg-rose-950/20 px-3 py-2 text-right text-xs font-bold tabular-nums text-rose-400">{{ fmt(row.total) }}</td>
                    ...
                    <tfoot>
                        <tr class="border-t-2 border-slate-700 bg-slate-800/60">
                            ...
                            <td
                                v-for="h in horas"
                                :key="h"
                                class="border-r border-slate-700/50 px-2 py-3 text-center text-xs font-semibold tabular-nums text-rose-400"
                            >
                                {{ fmt(totalesHora[h]) }}
                            </td>
                            <td class="border-r border-slate-700 bg-rose-950/40 px-3 py-3 text-right text-sm font-black tabular-nums text-rose-300">{{ fmt(totalGeneral) }}</td>
