...
function exportar() {
    ...
    const th = (t: string) =>
        `<th style="background:#1e3a5f;color:#fb7185;padding:6px 10px;border:1px solid #334155;white-space:nowrap">${t}</th>`;
    ...
    const filas = datosFiltrados.value
        .map(
            (r) =>
                `<tr>${td(r.item)}${td(r.modelo)}${td(r.centro)}${td(r.serie, 'font-family:monospace;color:#22d3ee')}${horasArr.map((h) => td(n(r[`h${h}`]), 'text-align:right')).join('')}${td(n(r.total), 'text-align:right;font-weight:bold;color:#fb7185')}${td(n(r.promedio), 'text-align:right;font-weight:bold')}</tr>`,
        )
        .join('');

    const totales = `<tr style="background:#1e293b;font-weight:bold">${td('')}${td('')}${td('')}${td('TOTAL', 'font-weight:bold')}${horasArr.map((h) => td(n(totalesHora.value[h]), 'text-align:right;color:#fb7185')).join('')}${td(n(totalGeneral.value), 'text-align:right;color:#fb7185;font-size:1.05em')}${td(n(promedioGeneral.value), 'text-align:right;color:#fbbf24;font-size:1.05em')}</tr>`;

    const html = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="utf-8"><style>body{font-family:Arial,sans-serif;font-size:11px;background:#0f172a;color:#e2e8f0}table{border-collapse:collapse;width:100%}</style></head><body><h3 style="color:#fb7185;margin-bottom:8px">Producción Colombia · ${fechaInicio.value} → ${fechaFin.value} ${tc ? ` · TC: ${tc}` : ''}</h3><table>${encabezado}${filas}${totales}</table></body></html>`;
    ...
    a.href = url;
    a.download = `produccion_colombia_${fechaInicio.value}_${fechaFin.value}${tc ? `_TC${tc}` : ''}.xls`;
    a.click();
        ...
        <div class="min-h-screen bg-[#080c18] font-sans text-white">
            <div class="mx-auto max-w-[1700px] px-2 py-4 sm:px-4 sm:py-6">
                ...
                >
                    <div>
                        <p
                            class="mb-1 text-xs font-semibold tracking-[0.2em] text-rose-500 uppercase"
                        >
                            Producción — Colombia
                        </p>
                        <h1
                        ...
                        >
                            Reporte
                            <span class="text-rose-400">por Centro</span>
                        </h1>
                ...
                <div class="mb-4 grid grid-cols-2 gap-3 sm:mb-6 sm:grid-cols-4">
                    ...
                    <div
                        class="rounded-xl border border-rose-900/50 bg-rose-900/20 p-3 sm:p-4"
                    >
                        <p
                            class="text-[10px] tracking-wider text-rose-600 uppercase sm:text-xs"
                        >
                            Total
                        </p>
                        <p
                            class="mt-1 text-xl font-black text-rose-400 sm:text-2xl"
                        >
                            {{ fmtTC(totalGeneral) }}
                        </p>
                    </div>
                ...
                <div class="mb-3">
                    <input
                        ...
                        class="w-full rounded-xl border border-slate-700 bg-slate-800/60 px-4 py-2.5 text-sm text-white placeholder-slate-600 focus:border-rose-500 focus:outline-none sm:w-80 sm:py-3"
                    />
                </div>
                    ...
                    <table class="w-full border-collapse text-sm">
                        ...
                        <thead>
                            <tr class="border-b border-slate-800">
                                ...
                                <th
                                    class="min-w-[80px] border-r border-slate-800 bg-rose-950/40 px-3 py-3 text-center text-[10px] font-bold tracking-widest text-rose-500 uppercase"
                                >
                                    Total
                                </th>
                                <th
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td
                                    class="min-w-[80px] border-r border-slate-800 bg-rose-950/20 px-3 py-2 text-right text-xs font-bold text-rose-400 tabular-nums"
                                >
                                    {{ fmtTC(row.total) }}
                                </td>
                                <td
                        ...
                        <tfoot>
                            ...
                            >
                                ...
                                <td
                                    v-for="h in horas"
                                    :key="h"
                                    class="border-r border-slate-700/50 px-2 py-3 text-center text-xs font-semibold text-rose-400 tabular-nums"
                                >
                                    {{ fmtTC(totalesHora[h]) }}
                                </td>
                                <td
