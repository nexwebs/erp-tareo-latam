...
function exportar() {
    window.location.href = `/produccion/excel?fecha=${fechaFin.value}&pais=colombia`;
}
    ...
    <div class="min-h-screen bg-[#080c18] font-sans text-white">
        <div class="mx-auto max-w-[1700px] px-4 py-6">
            ...
            <div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <div
                    class="rounded-xl border border-rose-900/50 bg-rose-900/20 p-4"
                >
                    <p class="text-xs tracking-wider text-rose-600 uppercase">
                        Total
                    </p>
                ...
                >
                    <p class="text-xs tracking-wider text-amber-600 uppercase">
                        Promedio/hora
                    </p>
                ...
                <table class="w-full min-w-max border-collapse text-sm">
                    <thead>
                        <tr class="border-b border-slate-800">
                            ...
                            <th
                                class="border-r border-slate-800 bg-rose-950/40 px-3 py-3 text-center text-[10px] font-bold tracking-widest text-rose-500 uppercase"
                            >
                                Total
                            </th>
                            <th
                    ...
                    <tbody>
                        ...
                        >
                            <td
                                class="sticky left-0 z-10 border-r border-slate-800/50 bg-[#080c18] px-3 py-2 text-center text-xs text-slate-600"
                            >
                                {{ row.item }}
                            </td>
                            <td
                                class="sticky left-10 z-10 w-[120px] max-w-[120px] truncate border-r border-slate-800/50 bg-[#080c18] px-3 py-2 text-xs text-slate-300"
                            >
                                {{ row.modelo }}
                            </td>
                            <td
                                class="sticky left-[130px] z-10 w-[180px] max-w-[180px] truncate border-r border-slate-800/50 bg-[#080c18] px-3 py-2 text-xs text-slate-300"
                            >
                                {{ row.centro }}
                            </td>
                            <td
                            ...
                            <td
                                class="border-r border-slate-800 bg-rose-950/20 px-3 py-2 text-right text-xs font-bold text-rose-400 tabular-nums"
                            >
                                {{ fmt(row.total) }}
                            </td>
                            <td
                    ...
                    <tfoot>
                        <tr class="border-t-2 border-slate-700 bg-slate-800/60">
                            ...
                            <td
                                class="border-r border-slate-700 bg-rose-950/40 px-3 py-3 text-right text-sm font-black text-rose-300 tabular-nums"
                            >
                                {{ fmt(totalGeneral) }}
                            </td>
                            <td
