...
const busqueda = ref('');
const metricaActiva = ref<
    'disponibilidad' | 'rendimiento' | 'eficiencia' | null
>(null);

const paisActual = computed(() => paisNombre[pais.value] ?? pais.value);

const diasRango = computed(() => {
    const f1 = new Date(fecha.value);
...
const fmt = (v: any) =>
    (parseFloat(v) || 0).toLocaleString('es-PE', {
...
const horasCeroMostradas = computed(() =>
    datosFiltrados.value.reduce(
        (s: number, r: any) => s + (parseInt(r.horas_con_produccion_cero) || 0),
        0,
    ),
);
...
const disponibilidadMedia = computed(() => {
    ...
    const horasTurno = maquinas * horasPorDia * diasRango.value;

    return horasTurno > 0
        ? (
              ((horasActivasMostradas.value + horasCeroMostradas.value) /
                  horasTurno) *
              100
          ).toFixed(1)
        : '0';
...
const rendimientoMedio = computed(() => {
    ...
    return (
        arr.reduce(
            (s: number, r: any) => s + (parseFloat(r.rendimiento) || 0),
            0,
        ) / arr.length
    ).toFixed(1);
});
...
const eficienciaRealMedia = computed(() => {
    ...
    return (
        arr.reduce(
            (s: number, r: any) => s + (parseFloat(r.eficiencia_real) || 0),
            0,
        ) / arr.length
    ).toFixed(1);
});
...
const metricas = computed(() => ({
    disponibilidad: {
        ...
        descripcion:
            'Mide qué fracción del turno de 16 horas (8h–23h) la máquina estuvo presente en el sistema, sin importar si produjo o no. Una hora con registro aunque sea de producción = 0 cuenta como disponible.',
        casos: [
            {
    ...
    rendimiento: {
        ...
        descripcion:
            'Compara la producción promedio del período contra el histórico de las últimas 4 semanas del mismo día de semana. Se limita a 100% — sobreproducir no incrementa este índice. Si no hay histórico y hay producción, se asume 100%.',
        casos: [
            {
    ...
    eficiencia: {
        ...
        descripcion:
            'Indicador compuesto que combina disponibilidad (¿estuvo conectada?) y rendimiento (¿produjo respecto a su histórico?). Si cualquiera de los dos es 0, la eficiencia real es 0. Ambos factores deben ser altos para un buen resultado.',
        casos: [
            {
            ...
            <div class="mx-auto max-w-[1900px] px-2 py-4 sm:px-4 sm:py-6">
                ...
                >
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <select
                            v-model="pais"
                            class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm focus:border-violet-500 focus:outline-none"
                            @change="filtrar"
                        >
                        ...
                        <button
                            @click="filtrar"
                            class="rounded-lg bg-violet-600 px-4 py-2 text-sm font-bold text-white hover:bg-violet-500 active:scale-95"
                        >
                            Filtrar
                        </button>
                        <button
                            @click="exportar"
                            class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-600 hover:border-slate-400 active:scale-95"
                        >
                            ↓ PDF
                        </button>
                    </div>
                ...
                <div class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-6">
                    ...
                    >
                        <p
                            class="text-[10px] tracking-wider text-blue-500 uppercase"
                        >
                            H. Cero
                        </p>
                        <p class="mt-1 text-2xl font-black text-blue-600">
                            {{ fmtInt(horasCeroMostradas) }}
                        </p>
                    </div>
                    ...
                    <table class="w-full border-collapse text-sm">
                        <thead>
                            <tr class="border-b border-slate-200 bg-slate-50">
                                ...
                                <th
                                    class="px-3 py-3 text-center text-[10px] font-bold tracking-widest text-blue-500 uppercase"
                                >
                                    H. Cero
                                </th>
                                <th
                                ...
                                <th
                                    class="cursor-pointer px-3 py-3 text-center text-[10px] font-bold tracking-widest uppercase transition-colors select-none"
                                    :class="
                                        metricaActiva === 'disponibilidad'
                                            ? 'bg-violet-50 text-violet-600'
                                            : 'text-emerald-600 hover:bg-slate-100 hover:text-violet-500'
                                    "
                                ...
                                <th
                                    class="cursor-pointer px-3 py-3 text-center text-[10px] font-bold tracking-widest uppercase transition-colors select-none"
                                    :class="
                                        metricaActiva === 'rendimiento'
                                            ? 'bg-violet-50 text-violet-600'
                                            : 'text-violet-500 hover:bg-slate-100 hover:text-violet-700'
                                    "
                                ...
                                <th
                                    class="cursor-pointer px-3 py-3 text-center text-[10px] font-bold tracking-widest uppercase transition-colors select-none"
                                    :class="
                                        metricaActiva === 'eficiencia'
                                            ? 'bg-violet-50 text-violet-600'
                                            : 'text-indigo-600 hover:bg-slate-100 hover:text-violet-700'
                                    "
                            ...
                            <tr v-if="metricaActiva">
                                <td colspan="14" class="p-0">
                                    <div
                                        class="border-b border-violet-200 bg-violet-50 px-5 py-4"
                                    >
                                        <div
                                        ...
                                        >
                                            <div>
                                                <p
                                                    class="text-[10px] font-semibold tracking-widest text-violet-400 uppercase"
                                                >
                                                    Cómo se calcula
                                                </p>
                                                <p
                                                    class="mt-0.5 text-sm font-black text-slate-800"
                                                >
                                                    {{
                                        ...
                                        <div
                                            class="mb-3 rounded-lg border border-violet-100 bg-white px-4 py-2 font-mono text-xs text-slate-700"
                                        >
                                            {{
                                        ...
                                        <p
                                            class="mb-3 text-xs leading-relaxed text-slate-600"
                                        >
                                            {{
                                                metricas[metricaActiva]
                                                    .descripcion
                                        ...
                                        <div class="grid grid-cols-3 gap-3">
                                            ...
                                            >
                                                ...
                                                <p
                                                    v-for="f in caso.filas"
                                                    :key="f"
                                                    class="font-mono text-[11px] leading-relaxed text-slate-500"
                                                >
                                                    {{ f }}
                                                </p>
                                                <div
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td
                                    class="px-3 py-2 text-right text-xs font-bold text-amber-600 tabular-nums"
                                >
                                ...
                                <td
                                    class="px-3 py-2 text-center text-xs font-semibold text-blue-500"
                                >
                                    {{ row.horas_con_produccion_cero }}h
                                </td>
                                <td
                                ...
                                <td
                                    ...
                                    :class="colorEficiencia(row.disponibilidad)"
                                >
                                    {{ row.disponibilidad }}%
                                </td>
                                <td
                                    class="px-3 py-2 text-center text-xs font-bold"
                                    :class="colorEficiencia(row.rendimiento)"
                                >
                                    {{ row.rendimiento }}%
                                </td>
                                <td
                                    class="px-3 py-2 text-center text-xs font-bold"
                                    :class="
                                        colorEficiencia(row.eficiencia_real)
                                    "
                                >
                                    {{ row.eficiencia_real }}%
                                </td>
                        ...
                        <tfoot>
                            <tr class="border-t-2 border-slate-200 bg-slate-50">
                                ...
                                <td
                                    class="px-3 py-3 text-center text-sm font-black text-blue-500"
                                >
                                    {{ fmtInt(horasCeroMostradas) }}h
                                </td>
                                <td
                                ...
                                <td
                                    class="px-3 py-3 text-center text-sm font-black"
                                    :class="
                                        colorEficiencia(
                                            parseFloat(disponibilidadMedia),
                                        )
                                    "
                                >
                                    {{ disponibilidadMedia }}%
                                ...
                                <td
                                    ...
                                    :class="
                                        colorEficiencia(
                                            parseFloat(eficienciaRealMedia),
                                        )
                                    "
                                >
                                    {{ eficienciaRealMedia }}%
