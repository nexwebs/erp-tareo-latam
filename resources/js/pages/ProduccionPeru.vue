...
function filtrar() {
    router.get('/produccion/peru', { fecha: fecha.value }, { preserveScroll: true });
}
...
function claseTotal(row: any): string {
    ...
    if (total >= promedio) return 'bg-blue-100 text-blue-700 font-bold';
        ...
        <div class="min-h-screen bg-slate-50 font-sans text-slate-700">
            <div class="w-full px-2 py-4 sm:px-4 sm:py-6">
                <header class="mb-6 flex flex-col gap-3 sm:mb-8 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <p class="mb-1 text-xs font-semibold tracking-[0.2em] text-emerald-500 uppercase">
                            Producción — Perú
                        </p>
                        <h1 class="text-2xl font-black tracking-tight text-slate-700 sm:text-3xl">
                            Reporte <span class="text-green-600">por Centro</span>
                        </h1>
                ...
                <div class="mb-4 grid grid-cols-2 gap-3 sm:mb-6 sm:grid-cols-4">
                    ...
                    <div class="rounded-xl border border-green-200 bg-emerald-900/20 p-3 sm:p-4">
                        <p class="text-[10px] tracking-wider text-green-700 uppercase sm:text-xs">Total</p>
                        <p class="mt-1 text-xl font-black text-green-600 sm:text-2xl">{{ fmt(totalGeneral) }}</p>
                    </div>
                    <div class="rounded-xl border border-amber-200 bg-amber-900/20 p-3 sm:p-4">
                        ...
                        <p class="mt-1 text-xl font-black text-amber-400 sm:text-2xl">{{ fmt(promedioGeneral) }}</p>
                    </div>
                    <div class="rounded-xl border border-slate-200 bg-white/40 p-3 sm:p-4">
                ...
                <div class="w-full overflow-x-auto rounded-2xl border border-slate-200/80 bg-white/60 pb-2 shadow-2xl">
                    <table class="w-full border-collapse text-sm" style="table-layout: auto;">
                        <thead>
                            <tr class="border-b border-slate-200">
                                <th class="sticky left-0 z-20 bg-white px-2 py-3 text-center text-[10px] font-bold tracking-widest text-slate-500 uppercase whitespace-nowrap">
                                    #
                                </th>
                                <th class="sticky z-20 bg-white px-3 py-3 text-left text-[10px] font-bold tracking-widest text-slate-500 uppercase whitespace-nowrap" style="left: 40px;">
                                    Modelo
                                </th>
                                <th class="sticky z-20 bg-white px-3 py-3 text-left text-[10px] font-bold tracking-widest text-slate-500 uppercase whitespace-nowrap" style="left: 170px;">
                                    Centro
                                </th>
                                <th class="sticky z-20 border-r border-slate-200 bg-white px-3 py-3 text-center text-[10px] font-bold tracking-widest text-slate-500 uppercase whitespace-nowrap" style="left: 330px;">
                                    Serie
                                </th>
                                <th
                                    v-for="h in horas"
                                ...
                                >{{ h }}h</th>
                                <th class="bg-blue-50 px-3 py-3 text-center text-[10px] font-bold tracking-widest text-blue-600 uppercase whitespace-nowrap">
                                    Total
                                </th>
                        ...
                        <tbody>
                            ...
                            >
                                <td class="sticky left-0 z-10 bg-slate-50 px-2 py-2 text-center text-xs text-slate-600 whitespace-nowrap">
                                    {{ row.item }}
                                </td>
                                <td class="sticky z-10 bg-slate-50 px-3 py-2 text-xs text-slate-300 whitespace-nowrap" style="left: 40px;">
                                    {{ row.modelo }}
                                </td>
                                <td class="sticky z-10 bg-slate-50 px-3 py-2 text-xs text-slate-300 whitespace-nowrap" style="left: 170px;">
                                    {{ row.centro }}
                                </td>
                                <td class="sticky z-10 border-r border-slate-200/50 bg-slate-50 px-3 py-2 text-center font-mono text-xs text-cyan-400 whitespace-nowrap" style="left: 330px;">
                                    {{ row.serie }}
                                </td>
                                <td
                                    v-for="h in horas"
                                ...
                                >{{ fmt(row[`h${h}`]) }}</td>
                                <td
                                    class="px-3 py-2 text-right text-xs tabular-nums whitespace-nowrap rounded-sm"
                                    :class="claseTotal(row)"
                                >{{ fmt(row.total) }}</td>
                                <td
                                    class="px-3 py-2 text-right text-xs tabular-nums whitespace-nowrap font-semibold text-slate-700 transition-colors rounded-sm"
                        ...
                        <tfoot>
                            <tr class="border-t-2 border-slate-700 bg-white/60">
                                <td
                                    colspan="4"
                                    class="sticky left-0 z-10 border-r border-slate-700 bg-white px-3 py-3 text-right text-xs font-black tracking-widest text-slate-700 uppercase whitespace-nowrap"
                                >Total</td>
                                <td
                                    v-for="h in horas"
                                    :key="h"
                                    class="border-r border-slate-700/50 px-2 py-3 text-center text-xs font-semibold text-green-600 tabular-nums whitespace-nowrap"
                                >{{ fmt(totalesHora[h]) }}</td>
                                <td class="bg-blue-50 px-3 py-3 text-right text-sm font-black text-blue-600 tabular-nums whitespace-nowrap">
                                    {{ fmt(totalGeneral) }}
                                </td>
                                <td class="bg-amber-950/40 px-3 py-3 text-right text-sm font-black text-amber-300 tabular-nums whitespace-nowrap">
                                    {{ fmt(promedioGeneral) }}
                                </td>
                            </tr>
                        </tfoot>
